...
<!-- InstanceBeginEditable name="head" -->
    <style type="text/css">
<!--
.form_style_user_detail {	line-height:20px;
	width:100%;
	text-align:left;
}
-->
...
<?		if($type == 'EDIT' && $_GET['id']) {
			$useracc_dtl = mysql_fetch_array(mysql_query('SELECT * FROM dex2_new_user_access WHERE id LIKE "'.$_GET['id'].'" LIMIT 0, 1'));
			$user_dtl = mysql_fetch_array(mysql_query("SELECT * FROM user WHERE username LIKE '".$useracc_dtl['USER']."'"));

			$acc_dtl = mysql_fetch_array(mysql_query('SELECT * FROM dex2_new_access WHERE UserGroupId LIKE "'.$useracc_dtl['id_access'].'" LIMIT 0, 1'));
			?>
            <div style='padding:5px;'>
              <div style='color:#630; font-weight:bold; font-size:20px; line-height:40px;'>ข้อมูล ผู้ใช้งานระบบสารบรรณ </div>
                <table border='0' class='tbl_style_02' style='width:100%;' cellpadding="0" cellspacing="0">
                    <tr>
                        <th style='text-align:right; width:120px;'>Username : </th>
                    ...
                    <tr>
                        ...
                        <td><? if($user_dtl['Birthdate'] != '0000-00-00') { ?> <? $set_date = new f_date; echo $set_date->date_th_l(strtotime($user_dtl['Birthdate']));?>
                            <? } else { ?> ไม่มีข้อมูล
                        <? }?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td><? if(!$org_dtl['Organize']) { ?>
                        ไม่มีข้อมูล
                        <? } else { echo $org_dtl['Organize']; }?></td>
                    </tr>
                    <tr>
                        <th style='text-align:right;'>User Access : </th>
                        <td>
                        	<?
							$acc_list_ = mysql_query('SELECT * FROM dex2_new_access'); ?>
                            <select id='slc_acc' name='slc_acc'>
                            	<option value=''>ยังไม่มีการกำหนดสิทธิการใช้งาน </option>
								<? for($a; $acc_list = mysql_fetch_array($acc_list_);) {?>
                                <option value='<?=$acc_list['UserGroupId'];?>'><?=$acc_list['GroupName'];?></option>
                                <? } ?>
                            </select>
                            
                            <script language="javascript">
								$('#slc_acc').val(<?=$acc_dtl['UserGroupId'];?>);
							</script>
	                    </td>                
                    </tr>
                    <tr>
                        ...
                        <td style='text-align:left;' colspan="2">
                        	<input type='submit' class='btn_style_01' value='บันทึกข้อมูล'/>
                        	<input type='button' class='btn_style_01' value='ย้อนกลับ' onclick='window.location="../member/user_document.php";'/>
                        </td>
                    </tr>
                </table>
			</div>
		<? } else {
			echo 'การเข้าถึงข้อมูลไม่ถูกต้อง';
		}
?>
</form>
<?	/*$_GET['tform'] = ($_GET['type'])?$_GET['type']:'ADD';

		$acclayer_list = mysql_query("SELECT * FROM dex2_user_group");
		$province_list = mysql_query("SELECT * FROM provinces");
