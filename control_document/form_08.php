<?	$old_data = mysql_fetch_array(mysql_query("SELECT * FROM dex2_new_document_08 WHERE id_doc1 LIKE '".$_GET['id']."' LIMIT 0, 1")); 
		$chk_lv5 = mysql_fetch_array(mysql_query("SELECT rdo_01 FROM dex2_new_document_05 WHERE id_doc1 LIKE '".$_GET['id']."' LIMIT 0, 1"));
?>
<table style='width:100%; padding:5px;'>
<tr><td colspan="2">
	<div style='color:#630; font-weight:bold; font-size:20px; line-height:40px;'>ลงเลขหนังสือ เสนอ ผอ. สท. </div>
...
<? if($form_type != 'VIEW') { ?><form method='post' action='process_08.php'> <? } ?>
	<input type='hidden' name='id' value='<?=$_GET["id"];?>'>

	<table cellpadding="0" cellspacing="0" style='width:100%;' class='tbl_style_02'>
        <tr>
            <th valign="top" style='width:240px;'>ผอ. สท. รับที่ *</th>
            <td><input type='text' name='txt_01' style='width:250px;' value='<?=$old_data['txt_01'];?>'/></td>
        </tr>
		<tr>
            <th>ลงวันที่</th>
            <td>
        ...
        <tr>
            ...
            <td>
                ...
                <? if($doc1_dtl['index_control'] == 8) { ?> <input style='margin-left:20px; ' type='checkbox' name='status_send' value='on' /> ส่งไปยังขั้นตอนถัดไป <? } ?>
            </td>
        </tr>
    </table>
